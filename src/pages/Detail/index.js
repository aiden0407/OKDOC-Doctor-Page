...
import {
  ...
  cancelCashlessTreatmentAppointment,
  cancelIosPurchase,
  cancelAndroidPurchase,
...
function Calendar() {
  ...
  const initHistoryList = async function (patientId) {
    try {
      ...
      puchaseHistory = puchaseHistory.filter(
        (obj) =>
          obj.fullDocument.treatment_appointment.doctor.id ===
          storedLoginData.id
      );
        ...
        try {
          ...
          if (document[document.length - 1].operationType === "delete") {
            ...
            if (obj.fullDocument?.appleIapUnsignedTransaction) {
              try {
                const response = await getIosAuditLog(obj.fullDocument.id);
                ...
                if (CanceledTime < wishAtTime) {
                  if (auditLog.principal?.role === "family")
              ...
              try {
                const response = await getAndroidAuditLog(obj.fullDocument.id);
                const auditLog = response.data.response[0];
                // 취소 시간과 role 데이터를 통한 취소 주체 규명
                const wishAtTime = new Date(
                  obj.fullDocument.treatment_appointment.hospital_treatment_room.start_time
      ...
      console.log("puchaseHistory", puchaseHistory);
        ...
        if (obj.STATUS === "RESERVED") {
          try {
            ...
            try {
              ...
              const response = await getTreatmentInformation(
                sessionToken,
                obj.fullDocument.treatment_appointment.id
              );
  ...
  function statusTranslator(item) {
    if (item.STATUS === "FINISHED" || item.STATUS === "ABNORMAL_FINISHED")
      return "진료 완료";
  ...
  const handleCancelTreatmentAppointment = async function (
    purchaseInformation
  ) {
    const result = window.confirm("해당 진료를 거절하시겠습니까?");
    if (result) {
      try {
        if (purchaseInformation?.appleIapUnsignedTransaction) {
          await cancelIosPurchase(purchaseInformation.id);
        } else if (purchaseInformation?.googleIapProductPurchase) {
          await cancelAndroidPurchase(purchaseInformation.id);
        } else {
          await cancelCashlessTreatmentAppointment(purchaseInformation.id);
        }
        alert("해당 진료가 정상적으로 취소되었습니다.");
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      <Row>
        <Text T2 bold color={COLOR.GRAY3}>
          {menuStatus}
        </Text>
        <Image src={arrowSolidIcon} width={32} marginLeft={10} />
        <Text T2 bold color={COLOR.MAIN} marginLeft={10}>
          진료 상세
        </Text>
      </Row>
      ...
      <PatientInfoContainer>
        <PatientInfoBox>
          ...
          <Row marginTop={30}>
            ...
            <ContentsText>
              <Text T5 medium>
                {patientData?.passport?.user_name ??
                  patientData?.passapp_certification?.name}
              </Text>
            </ContentsText>
          </Row>
          <Row marginTop={3}>
            ...
            <ContentsText>
              <Text T5 medium>
                {patientData?.passport
                  ? formatDate(String(patientData?.passport?.birth))
                  : patientData?.passapp_certification?.birthday.replaceAll(
                      "-",
                      "."
                    )}
              </Text>
        ...
        <PatientInfoBox>
          ...
          <Row marginTop={3}>
            ...
            <ContentsText>
              <Text T5 medium>
                {patientData?.drinker === "FREQUENTLY"
                  ? "자주"
                  : patientData?.drinker === "SOMETIMES"
                  ? "가끔"
                  : "안함"}
              </Text>
            </ContentsText>
          ...
          <Row marginTop={3}>
            ...
            <ContentsText>
              <Text T5 medium>
                {patientData?.drinker === "smoker" ? "흡연" : "비흡연"}
              </Text>
            </ContentsText>
          </Row>
        </PatientInfoBox>
      ...
      <Text T2 bold marginTop={54}>
        Consulting
      </Text>

      <ConsultingHeader>
        <ConsultingSection1></ConsultingSection1>
        <ConsultingSection2>
          <Text T5 bold>
            진료과 / 진료의
          </Text>
        ...
        <ConsultingSection3></ConsultingSection3>
      </ConsultingHeader>
          ...
          <div key={`consulting_${index}`}>
            ...
            >
              ...
              <ConsultingSection2>
                <Text T5>
                  {
                    item.fullDocument.treatment_appointment.doctor
                      .department_name
                  }{" "}
                  / {item.fullDocument.treatment_appointment.doctor.name} 님
                </Text>
              </ConsultingSection2>
              <ConsultingSection2>
                <Text T5>
                  {moment(
                    item.fullDocument.treatment_appointment
                      .hospital_treatment_room.start_time
              ...
              <ConsultingSection3>
                {
                  <StyledText T5>
                    {item?.treatment_data?.diseases?.[0]?.한글명}
                  </StyledText>
                }
              </ConsultingSection3>
              <ConsultingSection3 style={{ justifyContent: "flex-start" }}>
                {item.fullDocument.treatment_appointment.doctor.id ===
                  storedLoginData.id && (
                  <Row>
                    {item.STATUS === "RESERVED" && (
                      <ConsultingButton
                        disabled={enteranceTimeDisabled(
                          item.fullDocument.treatment_appointment
                            .hospital_treatment_room.start_time
                        )}
                        onClick={(e) => {
                          e.stopPropagation();
                          if (
                            !enteranceTimeDisabled(
                              item.fullDocument.treatment_appointment
                                .hospital_treatment_room.start_time
                            )
                          ) {
                            navigate(
                              `/telemedicine?id=${item.fullDocument.treatment_appointment.id}`
                            );
                          }
                        }}
                      >
                    ...
                    {item.STATUS === "IN_TREATMENT" && (
                      ...
                      >
                        <Text T6 color="#106DF9">
                          진료실 입장
                        </Text>
                      </ConsultingButton>
                    )}
                    {(item.STATUS === "FINISHED" ||
                      item.STATUS === "ABNORMAL_FINISHED") &&
                      !item?.treatment_data && (
                        <ConsultingButton
                          onClick={(e) => {
                            e.stopPropagation();
              ...
              >
                <Column style={{ width: "35%" }}>
                  <StyledRow>
                    ...
                    <FlexBox />
                    {item?.bidding_data?.attachments?.[0] && (
                      <Image
                        src={folderIcon}
                        width={25}
                        ...
                        <Column style={{ width: 90 }}>
                          <Row gap={4}>
                            <RadioButton
                              ...
                              value="임상적 추정"
                              checked={
                                item?.treatment_data?.diagnosis_type ===
                                "presumptive"
                              }
                            />
                            <Text T6>임상적 추정</Text>
